...
public class AppTest {
    ...
    public void urlsPageTest() {
        JavalinTest.test(app, (server, client) -> {
            ...
            var attributes = List.of("ID", "Имя", "Дата создания", "Код ответа");
            assertThat(response.code()).isEqualTo(200);
            assertThat(response.body().string()).contains(attributes);
        });
    }
    ...
    public void urlPageTest() {
        JavalinTest.test(app, (server, client) -> {
            ...
            assertThat(response.code()).isEqualTo(200);
            assertThat(response.getBody().string()).contains(url.getName());
            assertThat(response.getBody().string()).contains(url.getId().toString());
        });
    }
